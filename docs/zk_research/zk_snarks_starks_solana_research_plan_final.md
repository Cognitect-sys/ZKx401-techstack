...
| State tree | Concurrent Merkle tree | Parallelisme, skalabilitas state, root on-chip ringkas |
...
Sumber: Helius; Light Protocol; Solana PR.[^9][^14][^15][^16]
...
Sumber: Helius; Light Protocol; Solana PR; light-poseidon.[^9][^14][^17][^18][^12]
...
| Verifikasi on-chip | - | ~100k CU + ~100k CU sistem | Perlu kapasitas CU |

Sumber: Helius; Light Protocol.[^9][^8]

## Rekomendasi Strategis dan Playbook Implementasi

- Pilih SNARK ketika:
  - Verifikasi on-chip sering dan bukti harus sangat kecil (ZK Compression).
  - Throughput tinggi menjadi prioritas dan batas transaksi ketat (~1232 byte).
  - Platform membutuhkan interoperabilitas EVM (BN254) dan alat pairing yang mapan.[^7][^8]
...
- Pilih STARK ketika:
  ...
  - Pipeline verifikasi bertahap dan batching dapat mengurangi biaya on-chip.[^7][^10]
...
3. Setup pipeline prover (off-chip) dan verifikasi on-chip (alt_bn128 untuk SNARK).
4. Integrasikan state tree (Poseidon) dan rencana update root on-chip.
5. Operasional: menjalankan Photon RPC, prover nodes, light forester; monitoring syscalls dan feature gates.
6. Uji beban & pemulihan: fallback verifikasi off-chip, batching bukti, dan optimasi CU.
